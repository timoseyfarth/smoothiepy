...
from smoothiepy.filter.basefilter import Filter, Filter1D
...
class Smoother1D(Smoother, ABC):
    ...
    attachment of additional filters which must be of type BaseFilter1D.

    :ivar filter_list: Stores instances of BaseFilter1D for smoothing operations.
...
class Smoother1DContinuous(Smoother1D):
    ...
    This class extends the ` Smoother1D`` base class, providing functionality
    to smooth incoming data points in a continuous manner using a list of
    filters. The smoothed value is updated as new data points are added.
